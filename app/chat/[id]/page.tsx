...
export default function ChatPage() {
  ...
  const loadMessages = useCallback(() => {
    const savedMessages = JSON.parse(localStorage.getItem(`chat_${params.id}`) || "[]") as Message[]
  ...
  useEffect(() => {
    ...
    loadMessages()
  }, [checkActiveConsultation, loadMessages])
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      ...
      const newMessages = [...updatedMessages, doctorResponse]
      setMessages(newMessages)
      localStorage.setItem(`chat_${params.id}`, JSON.stringify(newMessages))
    }, 2000)
  }
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Chat Indisponível</h2>
            <p className="text-gray-600 mb-6">Você precisa ter uma consulta agendada para usar o chat.</p>
            <Button
              onClick={() => router.push(`/consultation-booking/${params.id}`)}
              className="bg-blue-600 text-white hover:bg-blue-700 rounded-full px-6 py-3 cursor-pointer"
            >
              Agendar Consulta
            </Button>
          </div>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white flex items-center justify-between p-4 shadow-sm border-b">
        ...
        <div className="flex space-x-2">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/call/${params.id}`)}>
            <Phone className="w-5 h-5" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => router.push(`/video-call/${params.id}`)}>
            ...
            <div key={message.id} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${
                  message.sender === "user" ? "bg-blue-600 text-white" : "bg-white text-gray-800 border"
                }`}
              >
